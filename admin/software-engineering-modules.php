...
// Fixed Course ID for Software Engineering = 3
$course_id = 3;

// 1. Fetch Software Engineering Course Info
...
$course_name = $courseData['course_name'] ?? 'BSc (Hons) in Software Engineering';
$course_code = $courseData['course_code'] ?? 'SE-2026';

// 2. Auto-Seeding Software Engineering Modules if Database is empty for Course ID 3
...
if ($totalCount == 0) {
    ...
    foreach ($seSubjects as $se) {
        $insStmt->bind_param("issss", $course_id, $se[0], $se[1], $se[2], $se[3]);
        $insStmt->execute();
    }
    $insStmt->close();
}

// 3. Fetch All Software Engineering Modules
